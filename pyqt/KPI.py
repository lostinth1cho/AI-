...
def proc_KPI(record_df, cash):
    ...
    # 累計報酬率
    acc_ROI = trade_df["獲利金額"].sum() / cash
    # 分割獲利交易資料與虧損資料
    ...
    trade_df["累計金額"] = trade_df["獲利金額"].cumsum() + cash
    ...
    # 將字典內容放進對應儲存格
    result_df.at['累計報酬率', "績效數值"] = output_KPI['累計報酬率']
    result_df.at['平均報酬率', "績效數值"] = output_KPI['平均報酬率']
    result_df.at['資產總額MDD', "績效數值"] = output_KPI['資產總額MDD']
    result_df.at['賺賠比', "績效數值"] = output_KPI['賺賠比']
    result_df.at['夏普指標', "績效數值"] = output_KPI['夏普指標']
    result_df.at['交易次數', "績效數值"] = output_KPI['交易次數']
    result_df.at['勝率', "績效數值"] = output_KPI['勝率']
    result_df.at['最大連續獲利次數', "績效數值"] = output_KPI['最大連續獲利次數']
    result_df.at['最大連續虧損次數', "績效數值"] = output_KPI['最大連續虧損次數']
    return result_df
